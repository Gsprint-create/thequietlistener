...
const FEELINGS = ["tired","alone","lonely","sad","overwhelmed","angry","stressed","worried","anxious","confused","lost","numb","guilty","ashamed","frustrated","hopeful","relieved","afraid","fear","hurt"];
...
function extractFeeling(text: string): string | null {
  const lower = text.toLowerCase(); return FEELINGS.find((w) => lower.includes(w)) || null;
}
function paraphrase(text: string): string {
  const t = text.replace(/\s+/g, " ").trim(); if (!t) return "";
  const max = 140; return t.length > max ? t.slice(0, max - 1) + "…" : t;
}
function makeListenerReply(userText: string): string {
  const feeling = extractFeeling(userText);
  const mirror = paraphrase(userText);
  const opener = feeling ? `I hear that you’re feeling ${feeling}.` : `I’m here with you. I hear you.`;
...
export default function App() {
  ...
  const [theme, setTheme] = useState<"light" | "dark">(
    (typeof window !== "undefined" && (localStorage.getItem(THEME_KEY) as "light" | "dark")) || "dark"
  );
  ...
  useEffect(() => {
    if (!incognito) {
      ...
      if (raw) {
        try { setMessages(JSON.parse(raw) as Msg[]); } catch {}
      }
    }
  }, [incognito]);
  ...
  async function handleSend() {
    ...
    setInput("");
    const now = Date.now();
    setMessages((m) => [...m, { id: `u-${now}`, role: "user", content: text, ts: now }]);

    setSending(true);
    await sleep(400 + Math.random() * 400);
    const reply = makeListenerReply(text);
    setMessages((m) => [...m, { id: `b-${Date.now()}`, role: "listener", content: reply, ts: Date.now() }]);
    setSending(false);
  ...
  function downloadTxt() {
    ...
    a.href = url; a.download = `no-judgment-transcript-${Date.now()}.txt`;
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100 transition-colors">
      ...
      <div className="mx-auto max-w-3xl px-4 pb-28">
        <div className="flex flex-col gap-3">
          <AnimatePresence>
            {messages.map((m) => (
              <motion.div key={m.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}>
                <MessageBubble msg={m} />
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          <div className="rounded-2xl border border-zinc-300/60 dark:border-zinc-700/60 bg-white dark:bg-zinc-900 shadow-sm">
            ...
            <div className="flex items-center justify-between px-3 pb-3">
              <div className="flex items-center gap-2 text-xs text-zinc-500">
                <span className="inline-flex items-center gap-1"><Lock className="w-3.5 h-3.5" /> {incognito ? "Incognito" : "Journal saved locally"}</span>
                <span>•</span>
